...
**Discipline.** Never leave unfilled gaps. When ingesting a source, follow every hyperlink — non-trivial citations become Tier 3+ candidates. Resolve `> [!question]` callouts when verification is cheap; mark exploratory only when verification is genuinely deferred. Don't let source pages sit orphaned (≥2 inbound wikilinks from concepts/topics).

**Queryable.** Every focus-area question must land on a `wiki/topics/` synthesis page within 1 hop. Test this when adding new content: walk in cold, ask the obvious question, confirm the path.
...
### Query-readiness pass (on request, or after big ingest rounds)
Walk each focus-area question through the wiki as a stranger: "how do I build better prompts?", "how do I save tokens?", "how do I reduce hallucinations?", etc. Confirm each lands on a `wiki/topics/` page within 1 hop. Any question that falls through to a stub or dead link is a flagged gap. Output: pass/fail per question + flagged gaps. Log as `query-readiness`.
...
- **log.md**: append-only. Header: `## [YYYY-MM-DD] <op> | <title>`. Ops: `ingest`, `query`, `lint`, `query-readiness`, `schema-change`, `manual`.
